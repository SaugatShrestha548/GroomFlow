...
// Deadline category colors (fixed, not user-chosen)
const DL_COLORS = {
  overdue:  { bg:"#2d0a0a", ring:"#ef4444", text:"#fca5a5" },
  urgent:   { bg:"#2d1a00", ring:"#f97316", text:"#fdba74" }, // ≤3 days
  warning:  { bg:"#2d2200", ring:"#eab308", text:"#fde047" }, // ≤7 days
  safe:     { bg:"#0a1a10", ring:"#22c55e", text:"#86efac" }, // >7 days
};
...
// ─── Deadline helpers ─────────────────────────────────────────────────────────
// Returns whole days between today-midnight and deadline-midnight.
...
// Fires routine reminder + deadline alerts each day at the set time
function scheduleDailyReminder(timeStr, tasks, deadlines) {
  ...
  return setTimeout(()=>{
    ...
    // Deadline notifications — fire for each deadline within 7 days (and overdue)
    const todayStr = toDateStr(today);
    deadlines.forEach(dl => {
      if (dl.done) return;
      const days = daysUntilDeadline(dl.date, today);
      if (days <= 7) {
        const label = dlBadgeLabel(days);
        fireNotification(`⏰ Deadline: ${dl.title}`, `${label} — ${dl.subject||"Assignment"}`);
      }
    });
...
function DeadlineForm({initial, onSave, onClose}) {
  ...
  function handleSave() {
    ...
    onSave({ title:title.trim(), subject:subject.trim(), date });
  }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div>{lbl("Assignment / Project title")}
        <input style={inp} placeholder="e.g. Data Structures Assignment 3" value={title} onChange={e=>setTitle(e.target.value)}/>
      </div>
      <div>{lbl("Subject / Course (optional)")}
        <input style={inp} placeholder="e.g. DSMA 122" value={subject} onChange={e=>setSubject(e.target.value)}/>
      </div>
      <div>{lbl("Deadline date")}
        <input type="date" style={inp} value={date} onChange={e=>setDate(e.target.value)}/>
      </div>
      <div style={{display:"flex",gap:12,paddingTop:4}}>
        <button onClick={onClose} style={{flex:1,padding:10,borderRadius:12,background:"transparent",
          border:"1px solid #2a2a2a",color:"#666",fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>Cancel</button>
        <button onClick={handleSave} disabled={!title.trim()||!date} style={{
          ...
          background:title.trim()&&date?"#c9273e":"#2a2a2a"}}>
...
function DeadlineTab({deadlines, onAdd, onEdit, onDelete, onToggleDone, today}) {
  ...
  // Sort: overdue first, then by soonest, done at bottom
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        {urgentCount>0&&(
          <span style={{fontSize:10,padding:"3px 10px",borderRadius:20,
            background:"#2d1a00",color:"#fdba74",border:"1px solid #f9731630"}}>
      ...
      {sorted.map(dl=>{
        ...
        const dateObj = parseDate(dl.date);
        ...
        return (
          <div key={dl.id} style={{...card,opacity:dl.done?0.5:1}}>
            <div style={{display:"flex",alignItems:"center",gap:12,padding:14}}>
              {/* Done toggle */}
              <button onClick={()=>onToggleDone(dl.id)} style={{
                background:"transparent",border:"none",cursor:"pointer",display:"flex",flexShrink:0,padding:2}}>
                {dl.done
                  ? <CheckCircle2 size={22} color="#22c55e" strokeWidth={1.8}/>
                  : <Circle size={22} color={c.ring} strokeWidth={1.8}/>}
              </button>
              {/* Info */}
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontSize:14,fontWeight:500,color:dl.done?"#555":"#fff",
                  textDecoration:dl.done?"line-through":"none",
                  overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{dl.title}</p>
              ...
              {/* Badge */}
              {!dl.done&&(
                <span style={{fontSize:10,padding:"3px 8px",borderRadius:20,whiteSpace:"nowrap",
                  background:c.bg,color:c.text,border:`1px solid ${c.ring}40`,marginRight:4}}>
                  {dlBadgeLabel(days)}
                </span>
              )}
              <button onClick={()=>setExpanded(isOpen?null:dl.id)} style={{
            ...
            {!dl.done && days>=0 && days<=7 && (
              <div style={{margin:"0 14px 10px",height:3,borderRadius:3,background:"#222"}}>
                <div style={{height:3,borderRadius:3,background:c.ring,
                  width:`${Math.round((days/7)*100)}%`,transition:"width 0.4s ease"}}/>
              </div>
            )}
            ...
            {isOpen&&(
              <div style={{padding:"0 14px 14px",borderTop:"1px solid #1e1e1e"}}>
                <div style={{display:"flex",gap:8,paddingTop:12}}>
                  <button onClick={()=>onEdit(dl)} style={{
                  ...
                  <button onClick={()=>{onDelete(dl.id);setExpanded(null);}} style={{
                    flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,
...
// Shows only deadlines within 7 days (and overdue), sorted by urgency
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        {urgent.map(dl=>{
          const days = daysUntilDeadline(dl.date, today);
          const c    = DL_COLORS[dlColorKey(days)];
          return (
            <div key={dl.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",
              borderRadius:14,background:c.bg,border:`1px solid ${c.ring}40`}}>
              ...
              <span style={{fontSize:11,color:c.text,whiteSpace:"nowrap",fontWeight:600}}>
                {dlBadgeLabel(days)}
              </span>
            </div>
          );
      ...
      {freqType==="weekly"&&(
        <div>{lbl("Repeat on")}
          <div style={{display:"flex",gap:6}}>
            {DAY_NAMES.map((dn,i)=>{const sel=days.includes(i);return(
              <button key={i} onClick={()=>toggleDay(i)} style={{flex:1,padding:"8px 0",fontSize:11,fontWeight:500,borderRadius:10,cursor:"pointer",fontFamily:"inherit",background:sel?"#3d0a14":"#1a1a1a",border:`1px solid ${sel?"#c9273e":"#222"}`,color:sel?"#f87185":"#555"}}>{dn}</button>
            );})}
          </div>
          ...
          <div>{lbl("Every how many days?")}
            ...
            <div style={{display:"flex",gap:6,marginTop:8}}>
              {[7,10,14,15,21,30].map(n=>(
                <button key={n} onClick={()=>setIntervalN(n)} style={{padding:"5px 10px",borderRadius:8,fontSize:11,cursor:"pointer",fontFamily:"inherit",background:Number(intervalN)===n?"#3d0a14":"#1a1a1a",border:`1px solid ${Number(intervalN)===n?"#c9273e":"#222"}`,color:Number(intervalN)===n?"#f87185":"#666"}}>{n}d</button>
              ))}
            </div>
          </div>
        ...
        <div>{lbl("Day of month")}
          ...
          <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>
            {[1,5,10,15,20,25,28].map(n=>(
              <button key={n} onClick={()=>setMonthDay(n)} style={{padding:"5px 10px",borderRadius:8,fontSize:11,cursor:"pointer",fontFamily:"inherit",background:Number(monthDay)===n?"#3d0a14":"#1a1a1a",border:`1px solid ${Number(monthDay)===n?"#c9273e":"#222"}`,color:Number(monthDay)===n?"#f87185":"#666"}}>{ordinal(n)}</button>
            ))}
          </div>
...
// ─── Today View ───────────────────────────────────────────────────────────────
function TodayView({tasks,today,checked,onToggle,deadlines}) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      {/* ── Deadline strip shown first if urgent ── */}
...
function NotifSettings({reminderTime,setReminderTime,notifStatus,onRequestPermission,tasks,deadlines}) {
  ...
  function sendTestNotif() {
    ...
    // Also test a deadline alert if any exist
    const urgentDl=deadlines.filter(d=>!d.done&&daysUntilDeadline(d.date,today)<=7)[0];
    if (urgentDl) {
      const days=daysUntilDeadline(urgentDl.date,today);
      setTimeout(()=>fireNotification(`⏰ Deadline: ${urgentDl.title}`,`${dlBadgeLabel(days)} — ${urgentDl.subject||"Assignment"}`),1000);
    }
  }
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{...card,display:"flex",alignItems:"center",gap:14}}>
        <div style={{width:40,height:40,borderRadius:12,background:"#1e1e1e",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
      ...
      <div style={{borderRadius:16,padding:14,background:"#111",border:"1px solid #1e1e1e"}}>
        <p style={{fontSize:11,color:"#666",lineHeight:1.7}}>
          ...
          Any deadline within 7 days triggers a daily notification until the deadline passes. Overdue deadlines also alert every day.
        </p>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // Count active urgent deadlines for badge on tab
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0f0f0f",fontFamily:"'DM Sans',sans-serif",paddingBottom:40}}>
      <div style={{position:"sticky",top:0,zIndex:40,background:"#0f0f0f",borderBottom:"1px solid #1a1a1a"}}>
        ...
        <div style={{maxWidth:460,margin:"0 auto",display:"flex",borderBottom:"1px solid #1a1a1a",marginTop:12}}>
          {tabs.map(({id,label,Icon,badge})=>(
            <button key={id} onClick={()=>setTab(id)} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:4,padding:"12px 0",fontSize:10,fontWeight:500,cursor:"pointer",background:"transparent",border:"none",fontFamily:"inherit",color:tab===id?"#f87185":"#555",position:"relative"}}>
              <Icon size={11}/>
              {label}
              {badge>0&&<span style={{position:"absolute",top:8,right:"18%",width:14,height:14,borderRadius:"50%",background:"#f97316",display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#fff",fontWeight:700}}>{badge}</span>}
              {tab===id&&<span style={{position:"absolute",bottom:0,left:"20%",right:"20%",height:2,borderRadius:2,background:"#c9273e"}}/>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{maxWidth:460,margin:"0 auto",padding:"20px 16px"}}>
        {tab==="today"     && <TodayView tasks={tasks} today={today} checked={checked} onToggle={handleToggle} deadlines={deadlines}/>}
